...
public class WagonOnSideOfTrail {
    ...
    public static void wagonOnSideOfTrail() {
        ...
        String person = "";
        boolean go = false;
        OregonTrailApplication.println("You come across an old abandoned wagon on the side of the trail. Should one go check it out?");
        int peopleCount = 0;
        if (!CharacterVariables.PERSON_1.IS_GONE) peopleCount++;
        if (!CharacterVariables.PERSON_2.IS_GONE) peopleCount++;
        if (!CharacterVariables.PERSON_3.IS_GONE) peopleCount++;
        if (!CharacterVariables.PERSON_4.IS_GONE) peopleCount++;
        String[] options = new String[peopleCount + 1];
        int index = 0;
...
//        switch (choice) {
//            case 0 -> OregonTrailApplication.println("You come to a forest; you pass it.");
//            case 1 -> {
...
//        }
    ...
    private static void goodWagon(String person) {
        ...
        OregonTrailApplication.println(person + " goes to check it out. " + person + " finds some bullets, medicine, and coins.");
        ...
        OregonTrailApplication.println("You continue traveling.");
    }
    private static void badWagon(String person) {
        ...
        OregonTrailApplication.println(personGetsSnakeBite + " goes to check out the wagon. " + personGetsSnakeBite + " finds some stuff, but suddenly a snake jumps out and bites them.\nThey run back to the wagon. " + personGetsSnakeBite + " is sick with a Snake Bite.");
